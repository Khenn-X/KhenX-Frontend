...
const scoreColor = (score?: number | null) => {
  if (score === undefined || score === null) return "bg-slate-500";
  if (score >= 7.5) return "bg-[#00C9A7]";
  if (score >= 5) return "bg-amber-500";
  return "bg-red-500";
};

const floodColor = (risk?: "low" | "medium" | "high" | null) => {
...
const ListingCard = ({
  ...
  const hasIntel =
    ...
    (intelligence.powerScore != null ||
      intelligence.securityScore != null ||
        ...
        {hasIntel && (
          <div className="absolute bottom-3 left-3 flex items-center gap-1.5">
            {intelligence?.powerScore != null && (
              <span
                className={cn(
                  "inline-flex shrink-0 items-center justify-center gap-1 whitespace-nowrap rounded-full px-2 py-1 text-[11px] font-bold leading-none text-white",
                  scoreColor(intelligence.powerScore),
                )}
              >
            ...
            {intelligence?.securityScore != null && (
              <span
                className={cn(
                  "inline-flex shrink-0 items-center justify-center gap-1 whitespace-nowrap rounded-full px-2 py-1 text-[11px] font-bold leading-none text-white",
                  scoreColor(intelligence.securityScore),
                )}
              >
            ...
            {intelligence?.floodRisk != null && (
              <span
                className={cn(
                  "inline-flex shrink-0 items-center justify-center gap-1 whitespace-nowrap rounded-full px-2 py-1 text-[11px] font-bold leading-none text-white",
                  floodColor(intelligence.floodRisk),
                )}
              >
        ...
        <div className="flex items-end justify-between">
          ...
          {listing.serviceCharge ? (
            <span className="text-[11px] text-slate-400">
              +{formatNaira(listing.serviceCharge)} SC
            </span>
          ) : null}
        </div>
