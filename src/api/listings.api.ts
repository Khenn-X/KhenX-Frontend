...
export const listingsApi = {
  ...
  initializeListingPlanSubscription: async (plan: string, returnUrl?: string) => {
    const { data } = await api.post('/listings/subscribe', {
      plan,
      returnUrl,
    });
  ...
  verifyListingPlanSubscription: async (reference: string, returnUrl?: string) => {
    const { data } = await api.get('/listings/subscribe/verify', {
      params: { reference, returnUrl },
    });
    return data.data as { plan: string; expiresAt: string; returnUrl: string };
  },

  getListingUsage: async (): Promise<ApiResponse<{ usage: {
    listingPlan: string;
    listingPlanStatus: string;
    listingPlanExpiresAt: string | null;
    quotaLimit: number;
    used: number;
    remaining: number | null;
    isUnlimited: boolean;
  } }>> => {
    const { data } = await api.get('/listings/usage');
    return data;
  },
};
