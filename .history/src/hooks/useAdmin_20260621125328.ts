...
export const useAdminStats = () => {
  return useQuery({
    ...
    staleTime: 1000 * 60 * 2,
    // Expose a flat `badges` shape so AdminLayout sidebar badges still work
    select: (res) => {
      ...
      return {
        ...
        data: {
          ...res.data,
          // Keep the nested shape for the dashboard page
          data: d,
          // Also expose flat keys for the sidebar badge helper
          stats: {
            pendingListings:   d?.listings?.pending     ?? 0,
            pendingKYC:        d?.agents?.pendingKYC    ?? 0,
            openFraudReports:  d?.fraud?.open           ?? 0,
          },
        },
      };
    },
