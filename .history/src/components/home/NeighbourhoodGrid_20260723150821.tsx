...
// ─── Pair chunking ────────────────────────────────────────────────────────────
// Every row after the top one repeats the same "big + medium" rhythm, instead
// of falling into a flat 3-across grid. Odd one out at the end gets a
// full-width row to itself rather than being stranded next to empty space.
...
export default function NeighbourhoodGrid() {
  ...
  const [hero, second, ...rest] = areas;
  const restPairs = chunkPairs(rest);
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {hero && (
                <div className="sm:col-span-2">
                  <AreaCard area={hero} size="hero" />
                </div>
              )}
              {second && <AreaCard area={second} size="medium" />}
            </div>

            {restPairs.map((pair, idx) => (
              <div key={pair[0]?._id ?? idx} className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {pair.length === 2 ? (
                  <>
                    <div className="sm:col-span-2">
                      <AreaCard area={pair[0]} size="hero" />
                    </div>
                    <AreaCard area={pair[1]} size="medium" />
                  </>
                ) : (
