...
export default function NeighbourhoodGrid() {
  const { data, isLoading, isError } = useFeaturedNeighbourhoods();
    ...
    <section className="py-12 bg-white">
      <PageWrapper>
        <div className="flex items-center justify-between mb-6">
          ...
          <a href="/neighbourhoods" className="text-sm text-[#00C9A7] font-semibold hover:underline">
            Compare areas
          </a>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            ...
            {rest.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {rest.map((a) => (
                  <AreaCard key={a._id} area={a} size="small" />
                ))}
              </div>
            )}
          </div>
