...
const adminNavItems = [
  { label: 'Dashboard',      to: ROUTES.ADMIN_DASHBOARD, icon: LayoutDashboard, statKey: null },
  { label: 'Listings',       to: ROUTES.ADMIN_LISTINGS,  icon: Building2,        statKey: 'pendingListings' },
  { label: 'KYC Verification', to: ROUTES.ADMIN_KYC,    icon: ShieldCheck,      statKey: 'pendingKYC' },
  { label: 'Fraud Reports',  to: ROUTES.ADMIN_FRAUD,     icon: AlertTriangle,    statKey: 'openFraudReports' },
  { label: 'Agents',         to: ROUTES.ADMIN_AGENTS,    icon: Users,            statKey: null },
] as const;

// Superadmin sees all admin items PLUS this section
const superadminOnlyItems = [
  { label: 'Admin Approvals', to: ROUTES.SUPERADMIN_ADMIN_REQUESTS, icon: ShieldCheck, statKey: null },
] as const;
...
const AdminLayout = ({ children }: AdminLayoutProps) => {
  ...
  // ✅ Fix: superadmin gets both sets; plain admin gets only admin items
  const navItems: NavItem[] = isSuperadmin
    ? [...adminNavItems, ...superadminOnlyItems]
    : [...adminNavItems];

  const getBadgeCount = (statKey: string | null): number | null => {
    if (!statKey || !stats) return null;
    const val = stats[statKey as keyof typeof stats];
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC]">
      ...
      >
        ...
        {/* Nav Items — ✅ uses `navItems` (role-aware), not hardcoded `adminNavItems` */}
        <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-1">
          {navItems.map(({ label, to, icon: Icon, statKey }) => {
            const badge = getBadgeCount(statKey);
            return (
              <NavLink
                key={to}
                to={to}
