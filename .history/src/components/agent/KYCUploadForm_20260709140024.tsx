...
const FileDropzone = ({ label, hint, file, onFile, onClear }: FileDropzoneProps) => {
  ...
  if (file) {
    return (
      <div className="flex items-center justify-between rounded-lg border border-[#00C9A7]/30 bg-[#00C9A7]/5 px-4 py-3">
        <div className="flex items-center gap-3">
          <FileCheck className="h-5 w-5 text-[#00C9A7] shrink-0" />
          <div>
    ...
    <div
      ...
      className={cn(
        ...
        dragging
          ...
          : 'border-slate-200 hover:border-[#00C9A7]/50 hover:bg-slate-50'
      )}
    >
...
const KYCUploadForm = () => {
  ...
  if (isSuccess) {
    return (
      <div className="flex flex-col items-center gap-4 rounded-xl bg-[#00C9A7]/5 border border-[#00C9A7]/20 p-8 text-center">
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <button
        type="submit"
        disabled={isPending}
        className="w-full rounded-lg bg-[#00C9A7] py-3 text-sm font-semibold text-[#0A1628] hover:bg-[#00b396] disabled:opacity-60 transition-colors"
      >
