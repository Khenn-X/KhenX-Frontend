...
import { KYCSubmission } from '../../api/kyc.api';
...
const KYCReviewCard = ({ submission, agentName, agentEmail }: KYCReviewCardProps) => {
  ...
  const handleReject = () => {
    ...
    reject({ agentId: submission.agentId, reason });
    setRejectMode(false);
    setReason('');
  };
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          <p className="font-semibold text-[#0F172A] truncate">{agentName || 'Unknown Agent'}</p>
          <p className="text-xs text-slate-400 truncate">{agentEmail}</p>
          <p className="text-xs text-slate-400">Submitted {timeAgo(submission.createdAt)}</p>
        </div>
      </div>

      {/* Documents */}
      {submission.kycDocuments.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Documents</p>
          <div className="flex flex-wrap gap-2">
            {submission.kycDocuments.map((url, i) => (
              <a
                key={i}
                href={url}
                target="_blank"
      ...
      {!rejectMode && (
        <div className="flex gap-2">
          <button
            onClick={() => approve(submission.agentId)}
