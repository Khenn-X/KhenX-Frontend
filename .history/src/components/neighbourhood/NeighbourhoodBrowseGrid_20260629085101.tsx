...
const formatRent = (min?: number | null, max?: number | null): string | null => {
...
const AreaCard = ({ area, variant }: CardProps) => {
  ...
  const heightClass =
    variant === 'wide'  ? 'h-64 sm:h-72'  :
    variant === 'tall'  ? 'h-64 sm:h-72'  :
                          'h-48 sm:h-52';
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {area.floodRisk && (
            <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold text-white ${
              area.floodRisk === 'low' ? 'bg-[#00C9A7]/80' :
              area.floodRisk === 'medium' ? 'bg-amber-500/80' :
              'bg-red-500/80'
            }`}>
              {area.floodRisk === 'low' ? 'Low Flood Risk' : area.floodRisk === 'medium' ? 'Medium Flood Risk' : 'High Flood Risk'}
            </span>
          )}
  ...
  if (pattern === 'A') {
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        {wide  && <div className="sm:col-span-2"><AreaCard area={wide}  variant="wide" /></div>}
...
const SkeletonRow = ({ pattern }: { pattern: 'A' | 'B' }) => {
  if (pattern === 'A') {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        <div className="sm:col-span-2"><Skeleton className="h-64 sm:h-72" /></div>
  ...
  if (isError) {
    return (
      <p className="text-center text-sm text-slate-500 py-10">
        Couldn't load neighbourhoods right now. Please try again shortly.
      </p>
    );
  }
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <h3 className="text-2xl sm:text-3xl font-bold text-[#0F172A]">Top Locales</h3>
        </div>
        <a
          href="/neighbourhoods/compare"
      ...
      {!isLoading && !hasMore && areas.length > 0 && (
        <p className="text-center text-xs text-slate-400 pt-2">
          All {areas.length} neighbourhoods loaded
        </p>
      )}
    </div>
  );
