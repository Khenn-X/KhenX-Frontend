...
import {
  ...
  Truck, Star, Quote, BadgeCheck, TrendingUp,
} from 'lucide-react';
...
const SuitabilityBar = ({ label, score }: { label: string; score: number }) => (
  <div>
    ...
    <div className="h-1.5 w-full rounded-full bg-white/10 overflow-hidden">
      <div className="h-full rounded-full bg-[#00C9A7]" style={{ width: `${(score / 10) * 100}%` }} />
    </div>
  </div>
);
...
const ListingCard = ({ listing }: { listing: typeof SAMPLE_LISTINGS[number] }) => (
  <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
    ...
    <div className="p-4">
      ...
      <p className="text-xs text-slate-400 mt-1">{listing.beds} Beds · {listing.baths} Baths · {listing.type}</p>
...
const AreaNotFound = ({ areaName }: { areaName: string }) => (
  <PageWrapper className="py-16">
    <Link to="/neighbourhood" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-[#0F172A] transition-colors mb-8">
...
export default function NeighbourhoodDetailPage() {
  ...
  const confidenceMap: Record<string, { label: string; bg: string; text: string }> = {
    low:    { label: 'Low confidence',    bg: 'bg-red-500/10 border-red-400/20',     text: 'text-red-300'     },
    medium: { label: 'Medium confidence', bg: 'bg-amber-500/10 border-amber-400/20', text: 'text-amber-300'   },
    high:   { label: 'High confidence',   bg: 'bg-[#00C9A7]/10 border-[#00C9A7]/25', text: 'text-[#00C9A7]'  },
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="relative min-h-[95vh] flex flex-col justify-end overflow-hidden bg-[#0A1628]">
        {area.imageUrl && (
          <img src={area.imageUrl} alt={area.areaName} className="absolute inset-0 h-full w-full object-cover scale-105" />
        )}
        ...
        <PageWrapper className="relative z-10 w-full pb-12 pt-28">
          <Link to="/neighbourhood" className="inline-flex items-center gap-2 text-sm text-slate-300 hover:text-white transition-colors mb-10">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-8">
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm text-slate-300 tracking-wide">Lagos, Nigeria</span>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3 mt-5">
                {rent && (
                  <div className="inline-flex items-center gap-1.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/15 px-4 py-2">
                    ...
                    <span className="text-sm text-white/85 font-medium">{rent}</span>
                  </div>
                )}
            ...
            <div className="flex sm:flex-col items-center sm:items-end gap-3 shrink-0">
              {area.overallScore != null && (
                <div className="flex flex-col items-center justify-center h-24 w-24 rounded-full border-2 border-[#00C9A7]/50 bg-white/5 backdrop-blur-sm shadow-[0_0_30px_rgba(0,201,167,0.15)]">
                  ...
                  <span className="text-[9px] text-slate-300 uppercase tracking-widest mt-1">Overall</span>
                </div>
              )}
              <span className={cn('rounded-full border backdrop-blur-sm px-3 py-1.5 text-xs font-semibold', confidence.bg, confidence.text)}>
                {confidence.label}
              </span>
            </div>
          </div>
      ...
      <PageWrapper className="py-10 space-y-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="flex gap-2">
              <button className="flex-1 rounded-lg border border-slate-200 py-2 text-xs font-semibold text-[#0A1628] hover:border-[#00C9A7]">Preview</button>
              <button className="flex-1 rounded-lg bg-[#0A1628] py-2 text-xs font-semibold text-white">PDF Report</button>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full bg-amber-100 text-amber-700 text-[9px] font-bold px-2 py-0.5">COMING SOON</span>
              </div>
        ...
        <div className="rounded-2xl bg-[#0A1628] p-6">
          ...
          <p className="text-center mt-4 text-xs font-semibold text-[#00C9A7]">{area.areaName} Market Snapshot</p>
        </div>
