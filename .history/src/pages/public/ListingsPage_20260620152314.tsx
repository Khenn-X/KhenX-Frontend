...
import ListingStatsBar from '../../components/listings/ListingStatsBar';
import ListingGrid from '../../components/listings/ListingGrid';
import Pagination from '../../components/shared/Pagination';
import PageWrapper from '../../components/layout/PageWrapper';

type ListingCategoryNavProps = {
  activeTab: string;
...
const listingCategoryTabs = [
  { id: '', label: 'All' },
  { id: 'land', label: 'Land' },
  { id: 'commercial', label: 'Commercial' },
  { id: 'short-let-tab', label: 'Short let' },
];
...
const ListingCategoryNav = ({
  ...
  <div className="space-y-4">
    ...
    <div className="flex flex-wrap gap-2">
      {listingQuickFilters.map((filter) => (
        <button
          key={filter.id}
          type="button"
...
const ListingsPage = () => {
  ...
  // NOTE: areaNames (multi-select), minPowerScore, maxFloodRisk, and features
  // aren't supported by the /listings endpoint yet — only the original
  // single-value fields (areaName, listingType, propertyType, bedrooms,
  // minPrice, maxPrice) are actually sent through and filtered server-side until
  // the backend is extended to accept the new ones.
  const { areaNames, minPowerScore, maxFloodRisk, features, ...serverFilters } = filters;
  ...
  const listings = Array.isArray(data)
    ...
    ? (data.data as any[])
    : Array.isArray((data as any)?.data?.listings)
    ? (data as any).data.listings
    : [];
  const meta = !Array.isArray(data) ? data?.meta ?? data?.data?.meta : undefined;
    ...
    <PageWrapper className="py-8 space-y-6">
      ...
      {hasSearched ? (
        ...
        <>
          ...
          {/* Stats bar */}
          <ListingStatsBar totalActive={meta?.total} />
